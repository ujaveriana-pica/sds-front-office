package co.edu.javeriana.pica.front.config;

import co.edu.javeriana.pica.front.entity.AuthUser;
import co.edu.javeriana.pica.front.exception.AuthException;
...
public class SecurityInterceptor implements ContainerRequestFilter {
    ...
    */
    ...
    private Optional<AuthUser> getUserInContext(ContainerRequestContext context) {
        String base64Token = context.getHeaderString(Constants.INTERNAL_TOKEN_HEADER);
    ...
    private static Optional<AuthUser> tokenToUser(String token) {
        return Optional.ofNullable(token).map(t -> {
            String[] arrTokens = t.split(Constants.INTERNAL_TOKEN_SEPARATOR);
    ...
    private static Optional<String> getInternalToken(String base64Token) {
        if(base64Token != null) {
            ...
            if(token.split(Constants.INTERNAL_TOKEN_SEPARATOR).length == 2) {
                return Optional.of(token.trim());
            } else {
                return Optional.empty();
            }
    ...
    public static AuthUser getAuthUser(HttpHeaders headers) {
        String base64Token = headers.getHeaderString(Constants.INTERNAL_TOKEN_HEADER);
        return getInternalToken(base64Token).flatMap(token -> tokenToUser(token))
